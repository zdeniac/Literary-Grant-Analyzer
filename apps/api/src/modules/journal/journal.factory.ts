...
export const createJournalModule = () => {
    const journoRepo = createJournalRepository(prisma.journal, true);
    const affiliationRepo = createJournalAffiliationRepository(prisma.journalAffiliation);
...
export const createJournalRepository = (entity: Database['journal'], withQueryBuilders?: boolean) => {
    ...
    return new JournalRepository(entity, crudRepo, listQb);
}

export const createJournalAffiliationRepository = (entity: Database['journalAffiliation']) => {
    const crudRepo = new PrismaCrudRepository(entity);

    return new JournalAffiliationRepository(entity, crudRepo);
}